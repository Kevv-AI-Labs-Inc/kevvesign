...
CREATE TABLE IF NOT EXISTS signing.packages (
 ...
 title TEXT NOT NULL, scenario TEXT NOT NULL CHECK(scenario IN ('onboarding','team_leader','buyer','seller','commercial','company_file')),
...
CREATE TABLE IF NOT EXISTS signing.requests (
 ...
 scenario TEXT NOT NULL CHECK(scenario IN ('onboarding','team_leader','buyer','seller','commercial','company_file','custom')),
...
DO $$
BEGIN
 IF EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='signing.packages'::regclass AND conname='packages_scenario_check' AND position('company_file' in pg_get_constraintdef(oid))=0) THEN
  ALTER TABLE signing.packages DROP CONSTRAINT packages_scenario_check;
  ALTER TABLE signing.packages ADD CONSTRAINT packages_scenario_check CHECK(scenario IN ('onboarding','team_leader','buyer','seller','commercial','company_file'));
 END IF;
 IF EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='signing.requests'::regclass AND conname='requests_scenario_check' AND position('company_file' in pg_get_constraintdef(oid))=0) THEN
  ALTER TABLE signing.requests DROP CONSTRAINT requests_scenario_check;
  ALTER TABLE signing.requests ADD CONSTRAINT requests_scenario_check CHECK(scenario IN ('onboarding','team_leader','buyer','seller','commercial','company_file','custom'));
 END IF;
END $$;
