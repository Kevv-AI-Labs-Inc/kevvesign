...
> **Product direction updated 2026-09-12.** Kevv eSign will be a Documenso-based API and integration tool for Portal: agent onboarding contracts, buyer/seller signing packages, and occasional custom documents. Documenso is the sole target signing engine. Native production is still running at this baseline; the replacement is not implemented yet. Follow [ADR 0002](docs/ADR/0002-documenso-portal-tool.md) and [the new plan](docs/DOCUMENSO_PORTAL_PLAN.md). Current native behavior documented below is historical implementation context, not the target roadmap.
