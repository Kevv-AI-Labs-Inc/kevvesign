...
- [ ] 选择并固定通过测试的 Documenso 版本与镜像摘要；核对其实际 OpenAPI，不能混用当前网站说明和现有 v2.11 实例的行为。
- [ ] 使用合成文档跑通双人多文件签约、退出再进入、签完下载、催签和回调。
- [ ] 证明 API 创建包可由正确经纪人在原生界面继续编辑，个人上传文档可回到 Portal 列表；验证 A 看不到 B 的文档和 HR 文件。
- [ ] 确认原生编辑/登录即可满足首期目标；如需付费功能，记录确切功能、版本和成本后再决定是否采购。
- [ ] 验证登录恢复、同一任务继续签署、Portal 文件工作台到原生编辑器的回程，不将原生后台首页链接当集成完成。
- [ ] 记录真实行为、缺口及最小改动，不因一个接口不合适而重建编辑器或引擎。
- [ ] 同时确定 Portal 的合同来源、收款事实、既有人员、例外能力、待办及统一鉴权模型；管理员流程是同轮实施范围。

交付物：三个合成场景的实际运行证据、选定版本、身份与所有权机制、准确的实施边界。

### Phase 1：准备 Documenso 生产运行与精简 API

- [ ] 使用生产身份、数据库、文件存储、邮件及签署证书；不要直接把当前开发实例当成生产。
- [ ] 实现最小映射库、唯一 Documenso client、包目录、幂等创建、回调及对账。
- [ ] 验证原件/完成件访问及持久化、重试、权限与错误提示。

### Phase 2：交付登录入职与管理员入职工作台

- [ ] 在 Documenso 建立公司选定的入职文件和角色，配置两个公司的包版本。
- [ ] 登录自动显示正确入职步骤，资料确认后准备包；发起/继续/重发/合同查看改走新 API，刷新不重复建包和发信。
- [ ] 新增 Portal 线下/历史合同登记核验、既有人员认定、有限例外所需数据与审计，适配合同要求判断。
- [ ] 解耦收款事实、应付匹配和开通资格，同时校准套餐/奖励副作用及防重复账务。
- [ ] 完成资料、合同、收款、权限四区面板、按未完成事项统计的队列和明确的下一步动作。
- [ ] 接入例外能力白名单、有效期与撤销，统一登录重定向、页面、API 和现有 session 的实时权限判断。
- [ ] 回归线上自动开通、线下收款审批、团队条款、已开通后的公司会签待办及普通编辑绕过保护。
- [ ] 验证纸质已签、既有人员、有限权限到期/补齐的完整操作链，再发布管理员工作台。
- [ ] 切断新任务进入 native 的路径。当前旧待办不迁移、不作为验收条件。

### Phase 3：交付买家/卖家包与个性化文件入口

- [ ] Portal 工作台新增独立“文件签署”入口 `/signing`：统一任务列表、买家包、卖家包、上传文件、搜索筛选与详情；入职继续由登录流程自动衔接。
- [ ] 配置公司提供的买家/卖家文件、角色和字段，完成预填与发送确认。
- [ ] 接入 Documenso 原生编辑，任务状态和下载回到 Portal。
- [ ] 管理员管理包配置，普通经纪人管理自己的文档，验证原生与 Portal 双侧权限一致。

### Phase 4：完成后删除重复实现并停止相关资源

- [ ] 删除 native 页面、签署 API、PDF finalizer、自建签署 workflow 和仅被它们引用的代码及依赖。
- [ ] 删除 native 默认值/回退逻辑，部署必须能证明新任务由 Documenso 创建并签署。
- [ ] 停止 native API/web/finalizer/workflow 等被替代服务；保留实际承担精简 API 的必要资源。
- [ ] 记录停止后的资源与成本变化，历史存储保留/删除不与代码退役混为同一动作。
- [ ] 重写部署、运维、验收和架构文档，归档旧计划。
...
- 现有入职文件在 Documenso 中的发布与字段配置。
- 所选 Documenso 版本、原生经纪人身份授权机制及可用的产品授权。
...
Portal 细节与未完成验收见相邻仓库 `docs/plans/2026-09-12-signing-onboarding-workspaces.md`。

## 生产候选已部署（23:10 EDT 更新）
